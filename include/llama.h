...
extern "C" {
    ...
    enum llama_ftype {
        ...
        LLAMA_FTYPE_MOSTLY_PQ2_0_LEGACY  = 142, // pre-rename file_type for the same format, still found in published ggufs
...
#define LLAMA_DECODE_EXECUTION_INTENT_VERSION 1u
    ...
    // Returns true if the model requires/supports a KV cache memory object
    LLAMA_API bool llama_model_has_kv_cache(const struct llama_model * model);
