...
class __Quant(ABC):
    ...
    def init_grid(cls):
        ...
        grid = grid.reshape((-1, 1)) >> np.arange(0, 8, 8 // elems_per_byte, dtype=np.uint8).reshape((1, elems_per_byte))  # Optimization: Use np.arange instead of np.array with list comprehension to avoid Python list allocation overhead
