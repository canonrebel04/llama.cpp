...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    ggml_tensor * moe_out;
    const uint32_t n_expert_used_ffn = hparams.n_expert_used(il >= 0 ? (uint32_t) il : 0);

    if (n_expert_used_ffn == 1) {
        ...
        for (uint32_t i = 0; i < n_expert_used_ffn; ++i) {
            cur_experts[i] = ggml_view_2d(ctx0, experts, n_embd, n_tokens, experts->nb[2], i*experts->nb[1]);
            ggml_build_forward_expand(gf, cur_experts[i]);
        }
        moe_out = cur_experts[0];
        for (uint32_t i = 1; i < n_expert_used_ffn; ++i) {
